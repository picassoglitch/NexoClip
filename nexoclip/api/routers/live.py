...
@router.get("/api/internal/live/destinations")
async def live_destinations(
    request: Request,
    stream_id: str,
    authorization: Annotated[str | None, Header()] = None,
) -> JSONResponse:
    """Multistream M2 — the relay fetches this stream's restream push
    targets (plaintext push URLs incl. stream keys) over the internal
    bearer. on_ready calls this, then spawns one `ffmpeg -c copy` per
    target.

    SECURITY: the response carries stream keys — the relay MUST NOT log it,
    and this endpoint never logs the URLs either.
    """
    _verify_internal_bearer(authorization)
    ...
    cur = await conn.execute(
        "SELECT tenant_id FROM streams WHERE id = ?", (sid,)
    )
    ...
    from nexoclip.destinations import resolve_targets
