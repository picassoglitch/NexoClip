...
async def test_dashboard_cancel_publish_job(
    client: httpx.AsyncClient,
    db: Database,
    tenants: dict[str, dict[str, str]],
) -> None:
    """Slice E.2: the dashboard's Undo button flips a pending job to
    canceled. The job stays in publish_jobs (for audit) but the worker
    skips it (status != 'pending')."""
    tenant_id = tenants["alice"]["id"]
    await _seed(
        db,
        tenant_id=tenant_id,
    ...
    # Log in via dashboard cookie + POST the cancel endpoint.
    await client.post("/dashboard/login", data={"token": tenants["alice"]["token"]})
    r = await client.post(
        f"/dashboard/publish-jobs/{job.id}/cancel",
        data={"redirect_to": "/dashboard"},
        follow_redirects=False,
    )
    assert r.status_code == 303
    with bound_tenant(tenant_id):
        after = (await PublishJobsRepo(db).list_for_clip("clp_a"))[0]
    assert after.status == "canceled"


async def test_dashboard_cancel_unknown_job_404(
    client: httpx.AsyncClient,
    tenants: dict[str, dict[str, str]],
) -> None:
    await client.post("/dashboard/login", data={"token": tenants["alice"]["token"]})
    r = await client.post(
        "/dashboard/publish-jobs/pjb_nope/cancel",
        data={"redirect_to": "/dashboard"},
        follow_redirects=False,
    )
    assert r.status_code == 404
...
async def test_dashboard_cannot_cancel_other_tenants_job(
    client: httpx.AsyncClient,
    db: Database,
    tenants: dict[str, dict[str, str]],
) -> None:
    """Bob can't cancel Alice's job even with the id."""
    tenant_id = tenants["alice"]["id"]
    await _seed(
        db,
        tenant_id=tenant_id,
